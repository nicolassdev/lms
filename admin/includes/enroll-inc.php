...
if (!isset($_POST["submit"])) {
    ...
    try {
        ...
        $insertEnrolled = "
        INSERT INTO `enroll` (`stu_lrn`, `section_code`, `semester`, `date_enroll`, `enroll_status`, `current_school`, `school_id`, `school_address`, `school_type`, `requirements`) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $mySQLFunction->con->prepare($insertEnrolled);

        // Bind the correct number of parameters
        $stmt->bind_param("ssssssssss", $student, $sec, $sem, $enrolldate, $status, $currentschool, $schoolid, $address, $type, $requirements);
