...
// Function to get the logged-in student's info
function getLoggedInStudent($conn, $studentId)
{
    $sql = "SELECT stu_fname, stu_lname FROM student WHERE stu_lrn = :stu_lrn";
    $stmt = $conn->prepare($sql);
    $stmt->execute(['stu_lrn' => $studentId]);
...
if (isset($_SESSION['stu_lrn'])) {
    // Get the latest student info from the database
    $studentInfo = getLoggedInStudent($conn, $_SESSION['stu_lrn']);

    // Update session variables with the latest data
    $_SESSION['stu_fname'] = $studentInfo['stu_fname'];
    $_SESSION['stu_lname'] = $studentInfo['stu_lname'];
} else {
